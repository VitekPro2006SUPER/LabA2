...
class IntegralApp:
    def __init__(self, root):
        self.root = root
        root.title("Лабораторна робота — Обчислення визначених інтегралів")
        root.geometry("850x560")

        # Верхня панель вибору функції
        ...
        # Ініціалізація меж для першої функції
        self.update_limits()
    ...
    # --- головна функція для обчислень і побудови ---
    def compute_and_plot(self, method):
        key = self.combo.get()
        data = FUNCTIONS[key]
        try:
        ...
        # побудова графіка (нове вікно)
        self.show_plot(method, func_np, func_scalar, a, b, true_val)

    # --- побудова графіка ---
    def show_plot(self, method, func_np, func_scalar, a, b, true_val):
        fig, ax = plt.subplots(figsize=(7, 5))
        xs = np.linspace(a, b, 1000)
        ys = func_np(xs)
        ax.plot(xs, ys, 'b', linewidth=1.8, label="f(x)")
        ax.grid(True)
        ax.set_title(f"{method.capitalize()} — {self.combo.get()}\nІнтервал [{a}, {b}]")
        ax.set_xlabel("x")
        ax.set_ylabel("f(x)")

        # зафарбована область під графіком
        ax.fill_between(xs, ys, alpha=0.15, color='skyblue')

        N_vis = 15  # кількість для наочного розбиття

        if method == "прямокутників":
            xs_div = np.linspace(a, b, N_vis + 1)
            ...
            ax.scatter(xs_rand[below], ys_rand[below], s=15, color='green', alpha=0.5, label='Під кривою')
            ax.scatter(xs_rand[~below], ys_rand[~below], s=15, color='red', alpha=0.5, label='Над кривою')
            ax.legend(["f(x)", "Точки Монте-Карло"])

        ax.text(0.02, 0.96, f"Аналітичне ≈ {true_val:.8f}", transform=ax.transAxes,
    ...
    def run_midpoint(self):
        self.compute_and_plot("прямокутників")

    def run_trapezoid(self):
        self.compute_and_plot("трапецій")

    def run_montecarlo(self):
        self.compute_and_plot("монте-карло")
